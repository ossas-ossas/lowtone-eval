 ...
 * 根据类别获取题目
 */
export function getItemsByCategory(category: string): DictionaryItem[] {
  return dictionaryData.filter(item => item.category === category);
}

/**
 * 获取所有题目
 */
export function getAllItems(): DictionaryItem[] {
 ...
 * 根据题目代码获取题目
 */
export function getItemByCode(itemCode: string): DictionaryItem | undefined {
  return dictionaryData.find(item => item.itemCode === itemCode);
}
